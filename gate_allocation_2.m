...
if str2double(messages_for_gate_maxchecker_unique(:,2)) >= 1   %>=1  % Atleast One project should be open to allot through gate scores
    ...
    BeforeOrAfter = 'After';
    [sorted_txt,~,~,~,~] =...
                max_proj_checker_deleter (projs_max_checker_3,max_proj_checker,N_students,sorted_txt,0);
    end
else
    
    disp('Need to call Manual allocation and need to specify the user that one project is not availabe and ask him not to allocate')
